...
function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data|| "An error occurred during resetting the password."); 
    }
  };
  ...
  return (
    <div className="reset-password-page">
      ...
      <select value={securityQuestion}
              ...
              <option value="What is your mother's name?">What is your mother's name?</option> </select>
      <input type="text" placeholder="Enter security answer" value={securityAnswer}
              onChange={(e) => setSecurityAnswer(e.target.value)}
              required />   
      <button type ="submit" className="button reset-password-button">Reset Password</button></form>
